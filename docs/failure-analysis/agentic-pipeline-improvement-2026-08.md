...
**Live flip subset (25 scenarios: 9 matcher-only + 3 NameError + 13 malformed + 2 controls), authoritative run_summary (out/agentic/live-g0/run_summary.json):** **25/25 completed, 14 passed / 11 failed (56%)**, raw-first-attempt 11, score_classes: 14 pass / 10 product_fail / **1 infra_blocked** (the reclassification working — a persistent empty now honestly infra, not product). The 2 controls pass, so **12 of the 23 previously-failing scenarios recovered (52%)**; the infra reclassification ALSO recovered 3 via the now-reachable retry.
- Matcher-only: 6/9 recovered; remaining 3 fail on structured/other grounds (assessment-level or a different flaky reply this run).
- Malformed: 3 recovered via the now-reachable retry (image-animatediff, video-wan2-2-high-low-noise, video-wan2-2-dual-noise-626); the real-token parser-contract case (multi-animated-4ed6d9) correctly still fails; the rest are persistent empties — retried, failed again, now honestly infra (1 infra_blocked).
- NameError class: gone from the failure mix (retry path works; residual fails are different classes).
