...
| `explain-comfy-workflow` | `vibecomfy inspect`, `vibecomfy analyze info`, `vibecomfy validate`, `vibecomfy nodes spec` | Available. Answers workflow questions from evidence without editing or running unless the user asks. |
...
| `edit-comfy-workflow` | `vibecomfy import`, `vibecomfy nodes reconcile`, Python `VibeWorkflow`, `vibecomfy validate`, `vibecomfy doctor` | Available. Load candidates through `load_bundle()` before editing. |
...
| `add-comfy-workflow-template` | `vibecomfy import` + manifest + `nodes reconcile` + `vibecomfy templates create` + tests | Available as a procedural template-addition workflow. |
