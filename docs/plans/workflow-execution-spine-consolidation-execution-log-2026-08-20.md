...
### RR3-ERRATUM — RR3-EVIDENCE receipt violation discharged as spurious — 2026-08-26

> [!NOTE]
> **Evidence dispatch only (§6).** Orchestrator-verified erratum transcribed into the durable record; this recorder judges no substance and records no own end_ts/receipt digest per brief. Wrapper receipts stay untracked run artifacts under `receipts/`. No credential material.

- **Firing record:** `RR3-EVIDENCE` (receipt `docs/plans/workflow-execution-spine-consolidation-evidence/receipts/RR3-EVIDENCE-receipt.json`, finished 2026-08-26T07:52:11Z, route `ox-alpha`, child exit 0) produced closeout commit `1764c627` (log section RR3-CLOSEOUT, report Addendum 2026-08-26, manifest seq[81]) — exactly the intended content, pushed to origin as part of `bea00563..1764c627`.
- **Spurious violation:** wrapper receipt status `allowance_violation` (`RR3-EVIDENCE-violation.json`). ROOT CAUSE IS ORCHESTRATOR-SIDE ALLOWANCE AUTHORING, not agent drift: the dispatched allowance (`/workspace/vibecomfy-exec-spine-20260820/g0/RR3-EVIDENCE-allowance.json`) combined a MUTATING allowed-list (4 evidence paths) with the §21.4 READ-ONLY marker `"forbidden": ["**"]`: `{"allowed":["docs/plans/workflow-execution-spine-consolidation-execution-log-2026-08-20.md","docs/plans/workflow-execution-spine-consolidation-evidence/manifest.json","docs/plans/workflow-execution-spine-consolidation-evidence/test-shards.json","docs/plans/workflow-execution-spine-consolidation-final-report-2026-08-25.md"],"forbidden":["**"]}`. The blanket-forbidden overrode the allowed-list, so all three committed files (execution log, manifest.json, final report) were flagged; all three WERE inside the allowed globs (changed ⊆ allowed verified).
- **Agent behavior fully correct:** one commit, 3 files, validator exit 0 pre- AND post-commit, tree clean.
- **Disposition:** violation DISCHARGED as spurious (authoring gap: read-only marker on a mutating card — same family as the R4 allowance-glob gap ruled confined by RR1-BATCH-REVIEW). No code or product artifact touched.
- **Companion dispatch:** `RR1-ASSESS-R3PASS` (receipt written 2026-08-26T07:31:29Z, `success`, read-only assessor) confirmed the four round-3 window passes — already recorded inside commit `1764c627`'s RR3-CLOSEOUT section.
- **This erratum's manifest/validation:** `G7` stays `status: open`, `disposition: pending`; label unchanged. `evidence_sequence` now **82 records** (81 prior + `82 RR3-ERRATUM`, role `evidence`, disposition `recorded`; canonical_slot `RR3-ERRATUM`; no receipt — evidence dispatch only). `tasks[5].recovery_note.sha256` refreshed to this log's new SHA-256 (validator-required per manifest recurrence rule, `ARTIFACT_DIGEST`); `section_sha256` refreshed to new T1.1→EOF section hash. `test-shards.json` byte-identical, untouched; final report NOT touched by this erratum.

### Controls (this erratum)

- ONE coherent commit authored by `POM <peter@omalley.io>` containing ONLY the execution log (this `RR3-ERRATUM` section) + `manifest.json` G7 `evidence_sequence[82]` + the validator-required `tasks[5].recovery_note.sha256`/`section_sha256` refresh; `test-shards.json` byte-identical, not rewritten; final report untouched. No push, no rebase/reset/stash/checkout/history ops, no new branches; wrapper/receipt files remain UNTRACKED run artifacts.
- **Validator proof:** `python3 scripts/validate_workflow_execution_spine_evidence.py docs/plans/workflow-execution-spine-consolidation-evidence/manifest.json` exits **0 on the pre-edit tree** AND **0 on the post-edit working tree** (both exits recorded here per brief).
- **Protected state / hygiene:** base `1764c627` IS HEAD at start of this append (`git merge-base --is-ancestor` exit 0); `final_five`, `TEST_SINGLETON` (`f7d6408e…` frozen at `54467724…`), and single authoritative live_run `T7.2-FINALE-SPLIT` untouched. Secret hygiene: no credential material anywhere in this erratum; historical secrets remain referenced only by pinned (lineno, sha256) identities. G7 stays open; **JUDGMENT_REQUIRED: none** (spurious-violation discharge is the orchestrator's ruling, recorded as given).
