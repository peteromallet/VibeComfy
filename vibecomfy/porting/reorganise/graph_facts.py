...
def extract_graph_facts(
    ...
    *,
    ...
    The UI JSON is ingested through :class:`UiGraphIndex` for scoped identity. The
    ...
    ledger = UiGraphIndex.ingest(ui_json)
...
def _ordered_nodes(ledger: UiGraphIndex) -> list[tuple[str, str, Mapping[str, Any]]]:
...
def _ordered_scopes(ledger: UiGraphIndex) -> list[tuple[str, ScopeState]]:
...
def _scope_furniture(ledger: UiGraphIndex) -> list[ScopeFurnitureFact]:
...
class UiGraphIndex:
    ...
    @classmethod
    def ingest(cls, raw_graph: Mapping[str, Any]) -> "UiGraphIndex":
...
__all__ = ["UiGraphIndex", "ScopeState"]
