"""Load/lower/validate/emit stage functions for the agent-edit pipeline.

These stages materialize the agent-authored Python scratchpad, lower the
resulting workflow when needed, validate the lowered workflow, and emit the
candidate UI payload used by downstream queue/readiness checks.
"""
...
from ..lowering import inject_lowering_provenance
...
def _stage_emit(state: AgentEditState, _context: TurnContext) -> StageResult:
    ...
    inject_lowering_provenance(state)
    return StageResult(
        stage="emit",
        ok=True,
        blocking=False,
