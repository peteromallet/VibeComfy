"""Agent-edit lowering provenance helpers."""
...
def build_lowering_recovery_entries(
...
def build_lowering_change_entries(
    lowering_evidence: list[dict[str, Any]] | tuple[dict[str, Any], ...],
) -> list[dict[str, Any]]:
    entries: list[dict[str, Any]] = []
    for item in lowering_evidence:
...
def build_lowering_audit_entries(
    lowering_evidence: list[dict[str, Any]] | tuple[dict[str, Any], ...],
) -> list[dict[str, Any]]:
    entries: list[dict[str, Any]] = []
    for item in lowering_evidence:
...
def inject_lowering_provenance(state: AgentEditState) -> None:
    if state.report is None or not state.lowering_evidence:
        state.lowering_recovery_entries = []
        return
    recovery_entries = build_lowering_recovery_entries(state.lowering_evidence)
    ...
    if isinstance(change_report, dict):
        change_report["lowered"] = build_lowering_change_entries(state.lowering_evidence)


__all__ = [
    "build_lowering_audit_entries",
    "build_lowering_change_entries",
    "build_lowering_recovery_entries",
    "inject_lowering_provenance",
]
