"""
Humanize/narrative rendering helpers (T-038 extraction of the edit_humanize fragment).
...
"""
from vibecomfy.ingest.normalize import door_get_links, door_get_widgets_values
import json
import os
import re
...
def _terminal_answer_message(state: AgentEditState) -> str | None:
    from ._frag_batch_reports import _BATCH_EXIT_NOOP  # T-038 late import: sibling cycle broken; resolved at call time
...
def _synthesize_post_validation_narrative(
    ...
    *,
    ...
    # T-039 late import: the provider seam is patched on the assembled edit
    # module (edit.run_model_turn) by tests and runtime code; resolve at call
    # time so the patch stays visible (matches the exec shared-namespace
    # semantics).
    from vibecomfy.comfy_nodes.agent.edit import run_model_turn
    if change_details is None:
        change_details = _change_details_payload(state, context)
...
def _synthesize_batch_repl_message(
    ...
    *,
    ...
    from vibecomfy.comfy_nodes.agent.edit import _resolver_candidates_from_batch_turns  # T-038 late import: exec'd-host fragment; resolved at call time
    lead = _landed_edit_lead(state)
    if failure is not None:
...
__all__ = (
     ...
     "_narrative_artifact_path", "_narrative_context_payload",
     "_narrative_issue_payloads", "_narrative_issue_summary",
     "_narrative_research_payload", "_narrative_revision_payload",
     "_narrative_stage_payload", "_narrator_message_from_response",
     "_narrator_system_message", "_narrator_user_message", "_net_field_changes",
     "_node_class_label", "_node_key_values", "_node_label_by_uid", "_node_phrase",
     "_noop_field_changes", "_operation_detail_payload",
     "_post_edit_reorganisation_advice_sentence", "_real_field_changes",
     "_resolve_endpoint_label", "_resolve_output_slot_name",
     "_revision_candidate_retry_hint", "_revision_rejected_candidate_message",
