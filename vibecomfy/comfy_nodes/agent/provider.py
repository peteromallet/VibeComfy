...
def _compact_batch_system_prompt(
    *,
    ...
    return (
        ...
        "- `schema_check(class_type=\"Class\", member_kind=\"input|widget|output\", member=\"name\")` for exact structural-member absence evidence\n"
        f"{tool_heading}{tool_catalog_docs(active_tool_phase)}\n"
        "- `python()` — view the current workflow Python\n"
        "- `done()` — commit landed edits\n"
        "- `refuse(kind=\"requires_custom_nodes\"|\"clarify\", missing_classes=[...], feature_absences=[{evidence_id: ...}], evidence=[...], message=\"...\")` — model-selected typed terminal; only after exact structured absence evidence\n"
        ...
        "fence. Never respond with only a fenced block; include `done()`, or use the "
        "model-selected typed action `refuse(kind=\"requires_custom_nodes\"|\"clarify\", "
        "missing_classes=[...], feature_absences=[{evidence_id: ...}], evidence=[...], message=\"...\")` "
        "only when exact structured absence evidence is present. Generic noop is never "
        "promoted, and failed edits cannot be laundered into refusal.\n\n"
...
def build_batch_messages(
    *,
    ...
    if research_only:
        ...
        "- `schema_check(class_type=\"Class\", member_kind=\"input|widget|output\", member=\"name\")` — exact structural-member lookup\n"
        f"{tool_heading}"
        ...
        "- `done()` — commit landed edits\n"
        "- `refuse(kind=\"requires_custom_nodes\"|\"clarify\", missing_classes=[...], "
        "feature_absences=[...], evidence=[...], message=\"...\")` — model-selected "
        "typed terminal. Use only after exact structured absence evidence is visible; "
        "the authority layer rejects invented/partial classes, present classes, or any "
        "refusal after a failed edit. A generic `done()`/noop is never promoted.\n\n"
        "Output rule: name output slots, e.g. `up.IMAGE`, never bare `up`.\n\n"
        f"{render_prompt_doc()}\n"
