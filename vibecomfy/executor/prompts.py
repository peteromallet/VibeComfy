...
_TWO_STEP_EXECUTE_SYSTEM_TEMPLATE = (
    ...
    "\n"
    "Your `reply` text IS the final message the user reads — it is delivered\n"
    "verbatim as the answer to their query.  Write it as a complete,\n"
    "self-contained, user-facing reply: state what you changed (citing the\n"
    "accepted Δ id) or what you found, in plain prose the user can act on.\n"
    "Do not write status notes, internal reasoning, or host-contract jargon;\n"
    "the user sees exactly this text.\n"
    "\n"
    ...
    "======================\n"
    "When a named class is PROVEN absent (a `node_schema`/`registry_lookup`\n"
    "result or a rejected add attempt), the submit MUST use\n"
    "`self_assessment.outcome = \"requires_custom_nodes\"` — never the generic\n"
    "`no_change`.  `no_change` means the graph was examined and left as-is\n"
    "with no blocker; a proved-missing custom node is a typed refusal.  Cite\n"
    "the failed lookup's evidence id in `claim_refs.evidence_ids` so the\n"
    "refusal is grounded, and state exactly which class is missing.  Do not\n"
    "invent widget names, do not accept `widget_N` positional refs, and do\n"
    "not claim a class is missing from an unverified assumption.\n"
    "\n"
    ...
    "  }\n"
    "  \\\"self_assessment\\\": {\\\"outcome\\\": \\\"edited|no_change|requires_custom_nodes|clarify\\\", \\\"confidence\\\":\n"
    "                      string, \\\"note\\\": string}.\\n"
    "__NON_EDIT_CONTRACT__"
    "SELF-CHECK\n"
    "==========\n"
    "Before SUBMIT, verify: (1) every cited id exists in this session's ledger — "
    "copy evidence ids VERBATIM from the bracketed [hivemind:...] id shown on "
    "each search hit; NEVER construct an evidence id from a Discord URL "
    "(the channel and message numbers in a discord.com link are not evidence ids). "
    "Discord message evidence ids are 19-digit numbers: if the id you copied has "
    "fewer than 19 digits, you dropped digits — re-copy the full id from the hit;\n"
...
_GROUNDED_ANSWER_GUIDANCE = (
    "\n"
    "GROUNDED ANSWER / NARROW ACT\n"
    "============================\n"
    "The render and the retained graph ARE evidence: visible node values,\n"
    "wiring, and topology are graph facts you may state and act on without\n"
    "outside citations.  When the graph licenses a concrete answer, ANSWER —\n"
    "do not punt to \"contact support\", \"edit manually\", or a clarifying\n"
    "question.  When the graph licenses a narrow graph-local edit (the exact\n"
    "binding and a named field with a value the intent calls for), PERFORM it\n"
    "with the typed edit tools; a same-class parameter edit is preferred over\n"
    "a no-op or a clarify.  In every submit: PRESERVE facts already supported\n"
    "by this session's graph/evidence; REMOVE or explicitly QUALIFY only the\n"
    "unsupported claims; cite ACTUAL evidence ids produced in THIS session\n"
    "(delta_ids / lens_fact_ids / evidence_ids) — never invented ids.  Never\n"
    "submit an answer that still fails grounding: if a claim cannot be\n"
    "supported, qualify it or drop it — the host fails closed on ungrounded\n"
    "submits, and an over-refusal that strips every grounded fact is just as\n"
    "wrong as an uncited claim."
)
