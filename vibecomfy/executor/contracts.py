...
# The executor is deliberately usable without importing the ComfyUI agent
# package.  These callables describe the small host seam needed by the plain
# orchestration layer; the ComfyUI-owned implementation lives in
# ``comfy_nodes.agent.executor_adapter`` and is loaded lazily by ``core``.
# Keeping the contract here also gives non-ComfyUI hosts an explicit injection
# surface instead of making them emulate provider/runtime module internals.
...
# Neutral wire value used by the executor when it must synthesize a failure.
# The ComfyUI adapter converts it to its compatibility enum at the boundary.
