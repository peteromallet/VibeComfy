...
from vibecomfy.porting.edit.ops import EditOpParseError, parse_edit_delta
...
def test_apply_delta_upsert_link_replaces_existing_subgraph_link_and_advances_counter() -> None:
    ...
    subgraph = next(
        ...
        if scope.get("name") == "HelperGraph"
    )
    assert subgraph["state"]["lastLinkId"] == 102
    new_link = next(link for link in subgraph["links"] if link["id"] == 102)
    assert new_link == {
        "id": 102,
        "origin_id": 1,
        "origin_slot": 0,
    ...
    assert 102 in source["outputs"][0]["links"]
...
def test_apply_delta_rejects_legacy_set_title_and_reorder() -> None:
    """set_title/reorder are rejected at parse; they never reach apply."""
    with pytest.raises(EditOpParseError, match=r"Unsupported edit op 'set_title'"):
        parse_edit_delta([{"op": "set_title", "target": ["", "5"], "title": "TestNode"}])
    with pytest.raises(EditOpParseError, match=r"Unsupported edit op 'reorder'"):
        parse_edit_delta(
            [{"op": "reorder", "target": ["", "1"], "axis": "widgets", "order": ["second", "first"]}]
        )
