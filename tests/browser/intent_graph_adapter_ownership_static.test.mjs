...
test("all 82 rows pass semantic anchors and exact invented mutations fail", async () => {
  const authority = JSON.parse(await readFile(nativeAuthorityLedgerPath, "utf8"));
  assert.equal(authority.rows.length, 82);
  ...
  assert.deepEqual(
    ...
    Array.from({ length: 84 }, (_, index) => `NGA-${String(index + 1).padStart(3, "0")}`).filter(
