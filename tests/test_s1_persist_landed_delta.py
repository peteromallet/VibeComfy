...
def test_s1_local_gate_pass_cannot_override_candidate_hash_mismatch():
    ...
    assert stamped["audit"]["rejected_candidate"]["graph"] == response["candidate"]["graph"]


def test_s1_batch_turn_landing_cannot_override_candidate_hash_mismatch():
    receipt = _receipt_error("candidate_hash_mismatch")
    response = {
        "ok": True,
        "outcome": {"kind": "candidate"},
...
def test_s1_batch_and_candidate_without_replay_match_fail_closed():
    receipt = _receipt_error("candidate_hash_mismatch")
    response = {
        "ok": True,
        "outcome": {"kind": "candidate"},
...
def test_s1_debug_gates_cannot_override_candidate_hash_mismatch():
    receipt = _receipt_error("candidate_hash_mismatch")
    response = {
        "ok": True,
        "outcome": {"kind": "candidate"},
