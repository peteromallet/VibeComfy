...
Lens: CONTRACTS & VERIFICATION ADEQUACY. Check acceptance criteria are mechanically testable and proportionate (not ceremonial); check the test matrix covers every agent-goal done criterion without redundant suites; identify any criterion with NO test mapped, or tests that could pass while the user-visible goal fails; check the submit-guard change cannot strand existing users who had legacy staged value stored.
