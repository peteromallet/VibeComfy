INDEPENDENT PRE-EXECUTION CONTRACT REVIEW — read-only. Verdict must be exactly PASS or a list of evidence-backed issues (<300 words).

Check operational completeness vs the agent goal AND directional alignment vs North Star across these artifacts (do not assume they agree — verify):
1. Whether NS/goal/tasklist dependencies, ownership boundaries, acceptance criteria, validation commands, and sync authorization are mutually consistent and complete;
2. Every task cites goal criterion + NS principle or says none;
3. Any [XHARD] tag without threshold evidence must be flagged (expect none);
4. Confirmation there is exactly ONE preference source of truth in the design and that classification 'all normal' is justified.
...
- B2-T3 paired staged/threaded/unset regressions incl. zero data-vibecomfy-executor-stage nodes in threaded; Working… present; meta chip gone; both switch directions via real handler; keyed-DOM invalidation; explicit staged fixtures for legacy tests. Checkpoint: paired evidence green.
## Batch 3 — validation + evidence
- B3-T1 run once: node --test tests/browser/*.mjs; make fast; check_ir_boundary.py → all exit 0, no weakening of unrelated assertions. [DC4]
- B3-T2 evidence under .oracle/evidence/onboarding-*.{log,md} + matrix mapping DC1–DC5 incl. NS disposition. [DC5]
